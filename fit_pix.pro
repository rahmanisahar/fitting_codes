...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
pro fit_pix,indep,h_indep,dep,mass_one=mass_one, out_fit,indep2
npar = N_params()
...
ENDIF ELSE BEGIN
   set_plot_ps,out_fit +'.ps'
   colours
   cgplot,logindep,logdep,psym=2,symsize = 0.15, title = 'K_S law fitting pix by pix method' ,xtitle='$\Sigma$!Igas!N (M$\sun$pc!E-2!N)',ytitle='$\Sigma$!ISFR!N (M$\sun$ yr!E-1!Npc!E-2!N)'
   oplot, [min(logindep),max(logindep)]*result[0]+const, linestyle=0, thick=1,colo=2
   endps
 p=plot(logindep,logdep,'6',$
       ...
       SYM_FILLED = 1, $
       SYM_FILL_COLOR = 'gold')
   p2=plot([min(logindep),max(logindep)]*result[0]+const,'r2',/overplot)
